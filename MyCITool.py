...
def set_container_port():
    container_port = input('{} {} \n'.format('Please insert container port number or press Enter for default value\nDEFAULT value is', container_port_default)) or container_port_default
    while True:
        try:
    	    container_port = int(container_port)
        except ValueError:
            container_port = input('Host port number should be an integer! You can press Enter for default value\n') or container_port_default
            continue
        else:
            break
...
def set_host_port():
    host_port = input('{} {} \n'.format('Please insert host port number (integer) or press Enter for default value\nDEFAULT value is', host_port_default)) or host_port_default
    while True:
        try:
    	    host_port = int(host_port)
        except ValueError:
            host_port = input('Host port number should be an integer! You can press Enter for default value\n') or host_port_default
            continue
        else:
            break
    return host_port
...
try:
    ...
    print('SUCCESS! The image was built successfully!')
